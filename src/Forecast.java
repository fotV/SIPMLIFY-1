...
public class Forecast {
	...
	double averageΜonthlyConsumption;
	...
	{    
	 ...
	 for(i=0; i<prod.size(); i-- )
	 {
		...
		averageΜonthlyConsumption = SupplierProduct. getAverageMonthlyConsumption();
		...
		{    
		   ...
		   amount = CalculateΟrder(stockAmount,expectedAmount,averageΜonthlyConsumption,Leadtime ,SafetyStock,maxStorageAmount,Per);
	...
	private double CalculateΟrder(double stockAmount,double expectedAmount,double averageΜonthlyConsumption,int Leadtime,double SafetyStock,double maxStorageAmount,double Per)
	{   
		if(type==true)
	    {
			if((stockAmount+expectedAmount-averageΜonthlyConsumption- Leadtime)<=SafetyStock)	
	        {
	    	   amount = SafetyStock+2*averageΜonthlyConsumption-stockAmount-expectedAmount+Leadtime;
		...
		{
			if((stockAmount+expectedAmount-averageΜonthlyConsumption- Leadtime)<= SafetyStock*Per)	
	        {
